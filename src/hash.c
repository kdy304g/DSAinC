...
{
    if (h1->key == h2->key && h1->hash == h2->hash && h1->equal == h2->equal) {
        return 1;
    } else {
        return 0;
    }
...
void map_add(Map *map, Hashable *key, Value *value)
{
    ...
    //printf("%d\n", index);
    // prepend the new node to the list in the map
    Node *list = prepend(key, value, map->lists[index]);
    // set the new node as head of the list in map
...
{
    int i;
    //traverse through lists and do list_lookup
    for(i=0;i<map->n;i++){
